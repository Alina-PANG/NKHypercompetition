...
public class Interdependence {
	...
	public Interdependence(int idx, int n, boolean[] deps) {
		// @arg n: N decision variables
		position = idx;
		dependencies = new boolean[n];
	...
	public String toString() {
		String retString = position + ": ";
		for (int i = 0; i < dependencies.length; i++) {
			if (dependencies[i] == true) {
				retString += "1\t";
			} else {
				retString += "0\t";
			}
	...
	public static void main(String args[]) {
		...
		j[3] = true;
		j[4] = true;
		Interdependence i = new Interdependence(5, 5, j);
